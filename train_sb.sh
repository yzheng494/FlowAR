...
PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True CUDA_VISIBLE_DEVICES=1 uv run torchrun --nproc_per_node=1 --master_port 7811 \
main_flowar.py \
...
--use_sb --sb_mode i2i --sb_prediction x0 --sb_beta_max 1.0 \
--online_eval --eval_freq 1 --num_images 200 --eval_bsz 16 --val_eval_freq 1 \
--wandb \
2>&1 | tee "$LOG_FILE"